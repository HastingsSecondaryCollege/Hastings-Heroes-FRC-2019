...
public class LimeLightTrackPIDToArea extends Command {
  ...
  */
  ...
  final double SLOW_TARGET_APPROACH_ZONE_AREA = 5.0; //Do slow approach if starting withion this zone 
  ...
  public LimeLightTrackPIDToArea() {
  ...
  public LimeLightTrackPIDToArea(boolean isAutonomous) {
    // Use requires() here to declare subsystem dependencies
    // eg. requires(chassis);
        ...
        if ((speed < SLOW_VELOCITY) &&
            (ta > SLOW_TARGET_APPROACH_ZONE_AREA) ) {
              ...
              if (isAutonomous) {
                  ...
                  drive_cmd = (DESIRED_TARGET_AREA_TELEOP - ta) * SLOW_DRIVE_K; //TELEOP go to 185mm short of target
